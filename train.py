...
MODEL_PATH = "saved_models/DNN_SAVE_898590"    # path to load/save
...
train_set = data_file(split = 'train', transform = transform, download = True)    # set these download flags to True for the first run if not already downloaded
train_loader = torch.utils.data.DataLoader(dataset = train_set, batch_size = BATCH_SIZE, shuffle = True)
test_set = data_file(split = 'test', transform = transform, download = True)
test_loader = torch.utils.data.DataLoader(dataset = test_set, batch_size = BATCH_SIZE, shuffle =False)
val_set = data_file(split = 'val', transform = transform, download = True)
